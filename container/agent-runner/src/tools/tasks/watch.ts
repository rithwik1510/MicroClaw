...
function resolveHeartbeatFilePath(): string {
  const groupDir =
    process.env.NANOCLAW_GROUP_WORKSPACE_DIR ||
...
function insertWatchInstruction(existing: string, instruction: string): string {
...
export async function executeRegisterWatch(
  ...
  if (instruction.length > 400) {
    return {
      ok: false,
      content:
        'Instruction too long. Keep heartbeat watches under 400 characters.',
    };
  }
