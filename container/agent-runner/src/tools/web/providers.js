...
import { extractPageText, fetchSearchResultContexts, getWebSession, openUrl, searchWeb as searchWithPlaywright, } from './playwright-executor.js';
...
function fetchProvider(ctx) {
    const explicit = (ctx.secrets?.WEB_FETCH_PROVIDER || '').trim().toLowerCase();
    if (explicit)
        return explicit;
    return 'auto';
}
function browserFallbackEnabled(ctx) {
    const explicit = (ctx.secrets?.WEB_BROWSER_FALLBACK || '').trim().toLowerCase();
    if (explicit)
        return explicit === 'playwright';
    const legacy = (ctx.secrets?.WEB_TOOL_PRIMARY || '').trim().toLowerCase();
    if (legacy === 'off')
        return false;
    return true;
}
function getSearxngBaseUrl(ctx) {
    return (ctx.secrets?.SEARXNG_BASE_URL ||
...
function maxSearchResults(ctx) {
    return Math.max(1, Math.min(10, positiveInt(ctx.secrets?.WEB_SEARCH_MAX_RESULTS, 5)));
}
function searchCacheTtlMs(ctx) {
    return positiveInt(ctx.secrets?.WEB_SEARCH_CACHE_TTL_MINUTES, 15) * 60_000;
}
function fetchCacheTtlMs(ctx) {
    return positiveInt(ctx.secrets?.WEB_FETCH_CACHE_TTL_MINUTES, 15) * 60_000;
}
function fetchMaxChars(ctx) {
    return Math.max(500, Math.min(50_000, positiveInt(ctx.secrets?.WEB_FETCH_MAX_CHARS, 12_000)));
}
function fetchMaxBytes(ctx) {
    return Math.max(20_000, Math.min(5_000_000, positiveInt(ctx.secrets?.WEB_FETCH_MAX_RESPONSE_BYTES, 2_000_000)));
}
...
function setCachedSearch(provider, query, results, ttlMs) {
    if (results.length === 0)
        return;
    const cache = readCache();
    pruneExpired(cache.search);
...
function getCachedFetch(provider, url, maxChars) {
    const cache = readCache();
...
function setCachedFetch(provider, url, maxChars, document, ttlMs) {
    if (!document.content)
        return;
    const cache = readCache();
    pruneExpired(cache.fetch);
...
function htmlToSearchResults(html) {
    const dom = new JSDOM(html);
    const document = dom.window.document;
    const cards = Array.from(document.querySelectorAll('.result, .web-result'));
    const results = [];
    for (const card of cards) {
        const link = card.querySelector('.result__title a, .result__a') ||
            card.querySelector('a[href]');
        const href = normalizeSearchHref(link?.href || '');
        const title = normalizeText(link?.textContent || '');
        if (!href || !title)
            continue;
        const snippet = normalizeText(card.querySelector('.result__snippet, .result-snippet')?.textContent || '');
        let source = '';
        try {
...
function normalizeSearxngResults(payload) {
    ...
    for (const item of p.results) {
        ...
        out.push({
            ...
            source,
            publishedAt: typeof item.publishedDate === 'string'
                ? item.publishedDate
...
export async function searchWithSearxng(query, ctx) {
    ...
    const results = keepAllowedResults(ctx, normalizeSearxngResults(payload)).slice(0, maxSearchResults(ctx));
    setCachedSearch('searxng', query, results, searchCacheTtlMs(ctx));
    return results;
}
export async function searchWithDuckDuckGoHtml(query, ctx) {
    ...
    const results = keepAllowedResults(ctx, htmlToSearchResults(text)).slice(0, maxSearchResults(ctx));
    setCachedSearch('duckduckgo', query, results, searchCacheTtlMs(ctx));
    return results;
}
export async function searchWithPlaywrightProvider(query, ctx) {
    const result = await searchWithPlaywright({
        state: ctx.webSession,
        query,
        timeoutMs: ctx.searchTimeoutMs,
    });
    ctx.webSession = result.state;
    const results = keepAllowedResults(ctx, result.results.map((item, index) => ({
        ...item,
        rank: index + 1,
...
export async function performSearch(query, ctx) {
    const provider = searchProvider(ctx);
    const chain = provider === 'searxng'
        ? ['searxng']
        : provider === 'duckduckgo'
            ? ['duckduckgo']
            : provider === 'playwright'
    ...
    for (const name of chain) {
        try {
            if (name === 'searxng') {
                ...
                if (results.length > 0)
                    return { provider: 'searxng', results, evidence: [] };
            }
            if (name === 'duckduckgo') {
                const results = await searchWithDuckDuckGoHtml(query, ctx);
                if (results.length > 0)
                    return { provider: 'duckduckgo', results, evidence: [] };
            }
...
function extractReadableDocument(html, url, maxChars) {
    const dom = new JSDOM(html, { url });
    const reader = new Readability(dom.window.document);
    const article = reader.parse();
    const title = normalizeText(article?.title || dom.window.document.title || '');
    const text = stripBoilerplate(article?.textContent ||
...
export async function fetchWithHttp(url, ctx, maxChars = fetchMaxChars(ctx)) {
...
export async function fetchWithPlaywrightProvider(url, ctx, maxChars = fetchMaxChars(ctx)) {
    const opened = await openUrl({
        state: ctx.webSession,
        url,
        timeoutMs: ctx.pageFetchTimeoutMs,
    });
    ctx.webSession = opened.state;
    const content = await extractPageText({
        state: ctx.webSession,
        timeoutMs: ctx.pageFetchTimeoutMs,
        maxChars,
    });
    ctx.webSession = getWebSession(ctx.webSession);
    return {
        url,
        finalUrl: opened.url,
