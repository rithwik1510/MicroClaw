import { getRestrictedDomains, isRestrictedUrl, looksLikeChallengePage, } from './policy.js';
import { closeWebSession, extractSearchResults, extractLinks, extractPageText, getWebSession, openUrl, } from './playwright-executor.js';
...
function markStep(ctx, isSearch) {
...
function actionTimeoutMs(ctx, action) {
    const stepTimeout = action === 'search' ? ctx.searchTimeoutMs : ctx.pageFetchTimeoutMs;
    return Math.max(300, Math.min(stepTimeout, remainingBudgetMs(ctx)));
}
function fallbackMode(ctx) {
    return (ctx.secrets?.WEB_TOOL_FALLBACK || 'none').trim().toLowerCase();
}
function isFallbackEnabled(ctx) {
...
function toSourceList(sources) {
    ...
    for (let i = 0; i < sources.length; i++) {
        ...
        lines.push(`${i + 1}. ${item.title || '(untitled)'} | ${item.url}${snippet}`.slice(0, 520));
    }
    return lines.join('\n');
}
function makeSearchContextBlock(input) {
    ...
    const evidenceBlock = input.fetched && input.fetched.length > 0
        ? [
            'Fetched pages:',
            ...input.fetched.slice(0, 3).map((item, index) => `${index + 1}. ${item.title || '(untitled)'} | ${item.finalUrl || item.url}\n${item.excerpt}`.slice(0, 2500)),
        ].join('\n\n')
        : '';
    const providerSummary = input.providerText && input.providerText.trim()
...
function setSession(ctx, state) {
    ctx.webSession = state;
}
function challengeResult() {
    return {
        ok: false,
        content: 'Web access challenge detected (captcha/login wall). Please retry or use a direct source URL.',
    };
}
async function reuseCurrentPageSnapshot(ctx) {
    if (!ctx.webSession)
        return null;
    try {
        ...
        setSession(ctx, getWebSession(ctx.webSession));
        if (!text)
            return null;
...
export async function executeWebSearch(args, ctx) {
    ...
    if (!query)
        return { ok: false, content: 'Missing required arg: query' };
    try {
        ...
        return {
            ok: false,
            content: 'Web search returned no structured results. Try a more specific query or direct URL.',
        };
    }
    catch (err) {
...
export async function executeWebFetch(args, ctx) {
    ...
    try {
        const result = await performFetch(url, ctx, maxChars);
        return {
            ok: true,
...
export async function executeWebOpenUrl(args, ctx) {
    const budget = markStep(ctx, false);
    if (budget)
        return budget;
    const url = asString(args.url);
...
export async function executeWebExtractText(args, ctx) {
    const budget = markStep(ctx, false);
    if (budget)
        return budget;
...
export async function executeWebGetLinks(args, ctx) {
    const budget = markStep(ctx, false);
    if (budget)
        return budget;
...
export async function executeWebClose(_args, ctx) {
    try {
...
export async function closeWebSessionFromContext(ctx) {
    await closeWebSession(ctx.webSession);
    ctx.webSession = undefined;
}
