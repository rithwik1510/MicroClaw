...
function buildQueryVariants(query) {
    ...
    if (!normalized)
        return [];
    const tokens = normalized.match(/[A-Za-z0-9][A-Za-z0-9+.#/-]*/g)?.map((token) => token) || [];
    ...
    const compactTokens = tokens.filter((token) => {
        ...
        if (/\d/.test(token))
            return true;
        if (token.length >= 4 && !stopWords.has(lower))
            return true;
        return false;
    });
    const compact = normalizeText(compactTokens.join(' '));
    return Array.from(new Set([normalized, compact].filter((value) => value && value.length >= 3)));
}
function queryTerms(query) {
    return (normalizeText(query)
        .toLowerCase()
        .match(/[a-z0-9][a-z0-9+.#/-]*/g) || []).filter((token) => token.length >= 3);
}
function scoreSearchResult(query, item, index) {
    ...
    if (/\b(openai|anthropic|google|github|artificialanalysis|livebench|swe-bench)\b/i.test(item.source)) {
        score += 10;
    }
...
function asSession(state) {
    if (!state || typeof state !== 'object')
        return {};
    return state;
}
export function getWebSession(state) {
...
export async function openUrl(input) {
    ...
    const title = normalizeText(await withTimeout(ensured.page.title(), input.timeoutMs, 'page.title'));
...
export async function searchWeb(input) {
    ...
    for (const variant of queryVariants) {
        for (const provider of providers) {
            ...
            const results = rerankSearchResults(variant, await extractSearchResults({
                state: opened.state,
                timeoutMs: input.timeoutMs,
                limit: 10,
            }));
...
export async function extractSearchResults(input) {
    ...
    const rawResults = await withTimeout(ensured.page.evaluate((maxItems) => {
        const seen = new Set();
        const results = [];
        const selectors = [
        ...
        for (const selector of selectors) {
            ...
            for (const anchor of anchors) {
                ...
                if (title.length < 12)
                    continue;
                const container = anchor.closest('article, li, div.result, div[data-testid], .result, .fdb') ||
        ...
        return results;
    }, limit), input.timeoutMs, 'extractSearchResults');
    ...
    return rawResults.filter((item) => !blockedHosts.has(hostFromUrl(item.url) || item.source || ''));
}
export async function extractPageText(input) {
    ...
    const raw = await withTimeout(ensured.page.evaluate(() => {
        const roots = Array.from(document.querySelectorAll('article, main, [role="main"], .article, .post'));
        const preferred = roots
            .map((node) => (node.textContent || '').replace(/\s+/g, ' ').trim())
            .filter((text) => text.length >= 200)
            .join(' ');
        return preferred || document.body?.innerText || '';
    }), input.timeoutMs, 'extractPageText');
...
export async function fetchSearchResultContexts(input) {
    ...
    for (const result of input.results.slice(0, maxResults)) {
        ...
        try {
            ...
            const title = normalizeText((await withTimeout(page.title(), pageBudgetMs, 'result page title')) || '') || result.title;
            const raw = await withTimeout(page.evaluate(() => {
                ...
                const preferred = roots
                    ...
                    .join(' ');
                return preferred || document.body?.innerText || '';
            }), pageBudgetMs, 'result page extract');
            const excerpt = trimBoilerplate(raw).slice(0, maxChars);
            if (!excerpt || looksLikeChallengePage(excerpt))
                continue;
            collected.push({
                ...result,
                pageTitle: title,
...
export async function extractLinks(input) {
    ...
    const links = await ensured.page.evaluate((maxLinks) => {
...
export async function closeWebSession(state) {
    ...
    try {
        if (page && !page.isClosed()) {
            await withTimeout(page.close({ runBeforeUnload: false }), 1500, 'page.close').catch(() => undefined);
        }
    }
    catch {
    ...
    try {
        if (context) {
            await withTimeout(context.close(), 1500, 'context.close').catch(() => undefined);
        }
    }
    catch {
        // ignore cleanup failures
    ...
    try {
        if (browser) {
            await withTimeout(browser.close(), 1500, 'browser.close').catch(() => undefined);
        }
    }
    catch {
        // ignore cleanup failures
