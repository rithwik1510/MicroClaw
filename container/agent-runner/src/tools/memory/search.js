...
function resolveSearchDir(subfolder) {
    const inputDir = process.env.NANOCLAW_IPC_INPUT_DIR || '/workspace/ipc/input';
    return path.join(path.dirname(inputDir), subfolder);
}
function resolveGroupFolder(ctx) {
    return (process.env.NANOCLAW_GROUP_FOLDER ||
        ctx.secrets?.NANOCLAW_GROUP_FOLDER ||
        '');
}
...
export async function executeMemorySearch(args, ctx) {
    ...
    const requestPath = path.join(resolveSearchDir('memory-search-requests'), `${requestId}.json`);
    const resultPath = path.join(resolveSearchDir('memory-search-results'), `${requestId}.json`);
    try {
