...
describe('DiscordChannel', () => {
  ...
  describe('text message handling', () => {
    it('responds to /chatid without enqueueing agent message', async () => {
      const opts = createTestOpts();
      const channel = new DiscordChannel('test-token', opts);
      await channel.connect();
    ...
    it('responds to /ping without enqueueing agent message', async () => {
      const opts = createTestOpts();
      const channel = new DiscordChannel('test-token', opts);
      await channel.connect();
    ...
    it('replies with guidance when message content is unavailable', async () => {
      const opts = createTestOpts();
      const channel = new DiscordChannel('test-token', opts);
      await channel.connect();
  ...
  describe('sendMessage', () => {
    ...
    it('prefers natural boundaries when splitting long replies', async () => {
      const opts = createTestOpts();
      const channel = new DiscordChannel('test-token', opts);
      await channel.connect();
      ...
      await channel.sendMessage('dc:1234567890123456', text);

      expect(mockChannel.send).toHaveBeenCalledTimes(2);
      const sent1 = mockChannel.send.mock.calls[0][0];
      const sent2 = mockChannel.send.mock.calls[1][0];
      expect(sent1.length).toBeLessThanOrEqual(2000);
